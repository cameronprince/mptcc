...
def SetScaling(led_n, scal):
    print(f"SetScaling led_n: {led_n} scal: {scal}")
...
def PULLUP_DOWN(pull):
    print(f"PULLUP_DOWN pull: {pull}")
    selectBank(ISSI3745_PAGE2)
    writeRegister8(ISSI3745_PULLUPDOWN, pull)
...
def Set_RED(led_n, color):
    print(f"Set_RED led_n: {led_n} color: {color}")
    selectBank(ISSI3745_PAGE0)
    writeRegister8(ISSI_LED_MAP[0][led_n], color)

def Set_GREEN(led_n, color):
    print(f"Set_GREEN led_n: {led_n} color: {color}")
    selectBank(ISSI3745_PAGE0)
    writeRegister8(ISSI_LED_MAP[1][led_n], color)

def Set_BLUE(led_n, color):
    print(f"Set_BLUE led_n: {led_n} color: {color}")
    selectBank(ISSI3745_PAGE0)
...
def readRegister8(reg):
    print(f"readRegister8 reg: {reg}")
    try:
        data = i2c.readfrom_mem(I2C_ADDR, reg, 1)
        print(f"readRegister8 data: {data[0]}")
        return data[0]
    except OSError:
        return 0xFF
