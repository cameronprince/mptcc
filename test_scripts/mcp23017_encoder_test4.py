...
async def chirp():
    ...
    await asyncio.sleep_ms(5)  # Short chirp
    pwm.duty_u16(0)  # Turn off

# Global state tracking
input_interrupt = None  # Will hold (intf, intcap) tuple or None
...
def input_callback(pin):
    ...
    input_interrupt = (intf, intcap)  # Store tuple of values

# Polling task
async def poll_inputs():
    global input_interrupt
    while True:
        if input_interrupt is not None:  # Check if values are present
            intf, intcap = input_interrupt  # Unpack tuple
            process_interrupt(intf, intcap)
            input_interrupt = None  # Reset to None after processing
